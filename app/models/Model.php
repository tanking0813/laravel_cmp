        ...
        if($with){
            if(is_array($with_filter) && !empty($with_filter)){
                ...
                if($ispage){
                    ...
                    })->skip($skip)->with($with)->paginate($ispage)->toArray();
                }else{
                    $data = $model->select($fields)->where(function($query) use($filter){        
                        foreach ($filter as $v) {
                            $query->where($v[0], $v[1], $v[2]);        
                        }              
                    })->take($limit)->with($with)->get()->toArray(); 
                }
            ...
            if($ispage){
                ...
                })->orderBy($order[0],$order[1])->paginate($ispage)->toArray();
            }else{
                $data = $model->select($fields)->where(function($query) use($filter){        
                    foreach ($filter as $v) {
                        $query->where($v[0], $v[1], $v[2]);        
                    }             
                })->take($limit)->orderBy($order[0],$order[1])->get()->toArray();
            } 
        }
        return $data;
    }
